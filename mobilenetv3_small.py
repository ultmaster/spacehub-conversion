...
for subtype in ['050', '075', '100']:
    ...
    for i in range(1 if subtype < '075' else 0, 7):
        channels[i] = make_divisible(channels[i] * int(subtype) / 100, 8)
    print(channels)
    ...
    file_paths = {
        '050': 'download/mobilenetv3_small_050_lambc-4b7bbe87.pth',
        '075': 'download/mobilenetv3_small_075_lambc-384766db.pth',
        '100': 'download/mobilenetv3_small_100_lamb-266a294c.pth'
    }

    official = torch.load(file_paths[subtype])
    ...
    # net.cuda()
    # evaluate_on_imagenet(net, 'bicubic', gpu=True, full=True, batch_size=256, num_workers=12)

    json.dump(arch, open(f'generate/mobilenetv3-small-{subtype}.json', 'w'), indent=2)
    json.dump(kwargs, open(f'generate/mobilenetv3-small-{subtype}.init.json', 'w'), indent=2)
    torch.save(state_dict, f'generate/mobilenetv3-small-{subtype}.pth')
